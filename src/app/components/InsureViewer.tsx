...
const ROW1 = [
  { name: "Life Cover",              price: "R0",  features: ["R100,000–R10,000,000 cover", "No medical exam for cover under R1M", "Beneficiary payout within 48 hours of claim approval"] },
  { name: "Disability Cover",        price: "R0",  features: ["Monthly income protection if you can't work", "Covers taxi drivers and informal economy workers", "Claims assessed within 5 business days"] },
  { name: "Funeral Plan",            price: "R85", features: ["Covers you and up to 9 family members", "Payout within 24 hours", "R5,000–R50,000 benefit", "Includes grocery and airtime allowance"] },
];
...
export function InsureViewer({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" style={{ background: "#fff", fontFamily: "'Segoe UI', Arial, sans-serif", fontSize: 15 }}>
      ...
      <section style={{ textAlign: "center", padding: "52px 24px 36px", background: "#f7f7f9" }}>
        <h1 style={{ fontSize: 30, fontWeight: 700, color: "#1e1e2e", marginBottom: 6 }}>Protect What Matters Most</h1>
      ...
      <div style={{ padding: "40px 32px 24px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }} className="biz-insure-row2">
          {ROW2.map((card, i) => <Card key={i} card={card} />)}
        </div>
        <style>{`@media(max-width:900px){.biz-insure-row2{grid-template-columns:1fr 1fr!important}}@media(max-width:600px){.biz-insure-row2{grid-template-columns:1fr!important}}`}</style>
      </div>
